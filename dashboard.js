...
function bind(){
  ...
  $("searchInput").oninput=filterGuests; $("guestFilter").onchange=filterGuests; $("venueMessageFilter").onchange=filterGuests;
...
function recent(){
  $("recentList").innerHTML=sortNewestFirst(data.rows).slice(0,5).map(r=>`<article class="recent-item"><div><strong>${esc(r.guestName)}</strong><div class="guest-meta">${esc(r.mobileNumber)} · ${esc(r.action)}</div></div><span class="badge ${r.attending==="Yes"?"accepted":"declined"}">${r.attending==="Yes"?"Accepted RSVP":"Declined RSVP"}</span></article>`).join("")||`<p class="empty-state">No RSVPs yet.</p>`;
}
function parseTimestampValue(value){if(value instanceof Date)return value.getTime();const text=String(value||"").trim();if(!text)return 0;const match=text.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{2,4})(?:[,\s]+(\d{1,2})(?::(\d{2}))?(?::(\d{2}))?\s*(AM|PM)?)?/i);if(match){const first=Number(match[1]),second=Number(match[2]);let day=first,month=second,year=Number(match[3]);if(year<100)year+=2000;if(second>12){day=second;month=first}let hour=Number(match[4]||0),minute=Number(match[5]||0),secondPart=Number(match[6]||0);const meridiem=String(match[7]||"").toUpperCase();if(meridiem==="PM"&&hour<12)hour+=12;if(meridiem==="AM"&&hour===12)hour=0;const date=new Date(year,month-1,day,hour,minute,secondPart);return Number.isNaN(date.getTime())?0:date.getTime()}const parsed=Date.parse(text);return Number.isNaN(parsed)?0:parsed}
function getRecordTimestamp(record,fields=["lastUpdated","timestamp"]){for(const field of fields){const value=parseTimestampValue(record?.[field]);if(value)return value}return 0}
function sortNewestFirst(records,fields=["lastUpdated","timestamp"]){return[...(records||[])].map((record,index)=>({record,index,time:getRecordTimestamp(record,fields)})).sort((a,b)=>b.time-a.time||a.index-b.index).map(item=>item.record)}
function filterGuests(){
  const q=$("searchInput").value.trim().toLowerCase(),f=$("guestFilter").value,vf=$("venueMessageFilter").value,sf=$("shareFilter").value,showGuests=vf!=="all"||sf==="all";
  filtered=showGuests?sortNewestFirst(data.rows,["lastUpdated","timestamp"]).filter(r=>{const s=`${r.guestName} ${r.mobileNumber} ${r.rsvpId}`.toLowerCase(),venueState=r.announcementStatus==="Sent"?"Shared":"Not Shared";return(!q||s.includes(q))&&(f==="all"||f==="accepted"&&r.attending==="Yes"||f==="declined"&&r.attending==="No"||f==="withChildren"&&r.attending==="Yes"&&r.children>0||f==="withoutChildren"&&r.attending==="Yes"&&r.children===0||f==="updated"&&String(r.action).toLowerCase().includes("updated"))&&(vf==="all"||r.attending==="Yes"&&venueState===vf)}):[];
  guests();
  invitations();
}
function guests(){
  ...
  const trash=`<svg viewBox="0 0 24 24"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  $("guestList").innerHTML=filtered.map(r=>`<article class="guest-card"><div class="guest-card-header"><div><h3>${esc(r.guestName)}</h3><p class="guest-meta"><strong>Mobile</strong> ${esc(r.mobileNumber)}</p></div><span class="badge ${r.attending==="Yes"?"accepted":"declined"}"><span>Status</span> ${r.attending==="Yes"?"Accepted RSVP":"Declined RSVP"}</span></div></div><div class="guest-stats"><div><span>Adults</span><strong>${r.adults}</strong></div><div><span>Children</span><strong>${r.children}</strong></div><div><span>Updated</span><strong>${shortTime(r.lastUpdated)}</strong></div></div><p class="guest-meta"><strong>Child Ages</strong> ${r.childAges.length?esc(r.childAges.join(", ")):"None"}</p><p class="guest-meta"><strong>RSVP ID</strong> ${esc(r.rsvpId)}</p><div class="card-actions"><button class="secondary-button icon-text-button call" data-mobile="${esc(r.mobileNumber)}"><span class="svg-icon">${call}</span>Call</button><button class="secondary-button icon-text-button edit" data-id="${esc(r.rsvpId)}"><span class="svg-icon">${edit}</span>Edit</button>${r.attending==="Yes"?`<button class="whatsapp-button icon-text-button msg" data-id="${esc(r.rsvpId)}"><span class="svg-icon">${message}</span>Venue Message</button>`:""}<button class="danger-button icon-text-button delete-rsvp" data-id="${esc(r.rsvpId)}"><span class="svg-icon">${trash}</span>Delete RSVP</button></div></article>`).join("");
  ...
  document.querySelectorAll(".delete-rsvp").forEach(b=>b.onclick=()=>openDeleteRsvp(data.rows.find(x=>x.rsvpId===b.dataset.id)));
}
...
function normalizeInviteShare(inv){return inv?.shareStatus==="Shared"?"Shared":"Not Shared"}
function computeInvitationMetrics(invitations){const list=invitations||[],shared=list.filter(inv=>normalizeInviteShare(inv)==="Shared");return{personalInvitesCreated:list.length,invitesShared:shared.length,awaitingResponse:shared.filter(inv=>inv.status==="Awaiting RSVP").length,notYetShared:list.filter(inv=>normalizeInviteShare(inv)!=="Shared").length}}
function applyInvitationMetrics(){if(!data)return;data.summary={...(data.summary||{}),...computeInvitationMetrics(data.invitations||[])}}
function approvedInvitationMessage(url){return`Ahoy, Mateys!\n\nCaptain Husain is turning ONE, and we’d be delighted to have you celebrate this special milestone with us.\n\nPlease take a moment to RSVP using the link below so we can make the necessary arrangements.\n\nRSVP here:\n${url}\n\nThe venue details will be shared once finalized.\n\nWe look forward to celebrating this special day with you.\n\nWith love,\nMurtaza, Batul, Husaina, Zahra & Captain Husain`}
function invitations(){
  const q=$("searchInput").value.trim().toLowerCase(),f=$("invitationFilter").value,sf=$("shareFilter").value,vf=$("venueMessageFilter").value,items=data.invitations||[],showInvitations=sf!=="all"||vf==="all";
  filteredInvitations=showInvitations?sortNewestFirst(items,["lastUpdated","createdTimestamp"]).filter(inv=>{const s=`${inv.guestName} ${inv.mobileNumber} ${inv.inviteToken||inv.token} ${inv.rsvpId}`.toLowerCase(),share=normalizeInviteShare(inv);return(!q||s.includes(q))&&(f==="all"||inv.status===f)&&(sf==="all"||share===sf)}):[];
  ...
  const message=`<svg viewBox="0 0 24 24"><path d="M4 5h16v11H8l-4 3V5Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><path d="M8 9h8M8 12h6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>`;
  const trash=`<svg viewBox="0 0 24 24"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  $("invitationList").innerHTML=filteredInvitations.map(inv=>{const shared=normalizeInviteShare(inv)==="Shared",token=esc(inv.inviteToken||inv.token),rsvp=data.rows.find(r=>r.rsvpId&&r.rsvpId===inv.rsvpId);return`<article class="invitation-card"><div class="invitation-card-header"><div><h3>${esc(inv.guestName)}</h3><p class="guest-meta"><strong>Mobile</strong> ${inv.mobileNumber?esc(inv.mobileNumber):"Not provided"}</p></div><div class="invitation-badges"><span class="badge ${inv.status==="Accepted"?"accepted":inv.status==="Declined"?"declined":"pending"}">${esc(inv.status)}</span><span class="badge ${shared?"sent":"pending"}">${shared?"Shared":"Not Shared"}</span></div></div><div class="invitation-details"><div><span>Invited</span><strong>${esc(invitationPeople(inv.adultsInvited,inv.childrenInvited))}</strong></div><div><span>Attending</span><strong>${inv.status==="Awaiting RSVP"?"-":esc(invitationPeople(inv.adultsAttending,inv.childrenAttending))}</strong></div><div><span>Sharing</span><strong>${shared?`Shared<br><small>${esc(shortDateTime(inv.lastSharedAt))}</small>`:"Not Shared"}</strong></div><div><span>Share Count</span><strong>${Number(inv.shareCount||0)}</strong></div></div><p class="guest-meta invite-link-meta"><strong>Invitation ID</strong> ${token}</p><div class="card-actions"><button class="secondary-button icon-text-button copy-invite" data-token="${token}"><span class="svg-icon">${copy}</span>Copy Invitation Link</button><button class="secondary-button icon-text-button native-share-invite" data-token="${token}"><span class="svg-icon">${share}</span>${shared?"Share Again":"Share Invitation"}</button><button class="whatsapp-button icon-text-button share-invite" data-token="${token}"><span class="svg-icon">${wa}</span>${shared?"Share Again":"Share on WhatsApp"}</button>${rsvp&&rsvp.attending==="Yes"?`<button class="whatsapp-button icon-text-button invite-msg" data-id="${esc(rsvp.rsvpId)}"><span class="svg-icon">${message}</span>Venue Message</button>`:""}<button class="secondary-button icon-text-button edit-invite" data-token="${token}"><span class="svg-icon">${edit}</span>Edit Invitation</button>${shared?`<button class="secondary-button icon-text-button unshare-invite" data-token="${token}">Mark Unshared</button>`:""}<button class="danger-button icon-text-button delete-invite" data-token="${token}"><span class="svg-icon">${trash}</span>${inv.status==="Awaiting RSVP"?"Delete Invitation":"Delete RSVP"}</button></div></article>`}).join("");
  ...
  document.querySelectorAll(".invite-msg").forEach(b=>b.onclick=()=>{const r=data.rows.find(x=>x.rsvpId===b.dataset.id);if(r)openGuestWhatsApp(r)});
  ...
  document.querySelectorAll(".delete-invite").forEach(b=>b.onclick=()=>openDeleteInvitation(invitationByToken(b.dataset.token)));
}
...
async function markInvitationShared(inv,{silent=false}={}){if(!inv)return false;const r=await post({mode:"markInvitationShared",adminKey:key,inviteToken:inv.inviteToken||inv.token});if(r.success&&r.invitation){Object.assign(inv,r.invitation);applyInvitationMetrics();summary();filterGuests();if(!silent)toast("Invitation marked shared.");return true}toast(r.message||"Unable to update sharing status.",true);return false}
async function markInvitationUnshared(inv){if(!inv)return;const r=await post({mode:"markInvitationUnshared",adminKey:key,inviteToken:inv.inviteToken||inv.token});if(r.success&&r.invitation){Object.assign(inv,r.invitation);applyInvitationMetrics();summary();filterGuests();toast("Invitation marked unshared.")}else toast(r.message||"Unable to update sharing status.",true)}
async function shareInvitationWhatsApp(inv){if(!inv)return;const u=requireInvitationUrl(inv);if(!u)return;const marked=await markInvitationShared(inv,{silent:true});if(!marked)return;const target=`https://wa.me/${inv.mobileNumber?phone(inv.mobileNumber):""}?text=${encodeURIComponent(approvedInvitationMessage(u))}`;const opened=window.open(target,"_blank");if(!opened)location.href=target;toast("Invitation marked shared.")}
async function nativeShareInvitation(inv){if(!inv)return;const u=requireInvitationUrl(inv);if(!u)return;try{if(!navigator.share)throw Error("native-share-unavailable");const response=await fetch(INVITATION_IMAGE_PATH,{cache:"no-store"});if(!response.ok)throw Error("image-unavailable");const blob=await response.blob(),file=new File([blob],"husain-invite.jpg",{type:blob.type||"image/jpeg"});if(!navigator.canShare||!navigator.canShare({files:[file]}))throw Error("file-share-unavailable");await navigator.share({files:[file],text:approvedInvitationMessage(u)})}catch(e){if(e&&e.name==="AbortError")return;await shareInvitationWhatsApp(inv);return}await markInvitationShared(inv)}
...
async function openGuestWhatsApp(r){if(!r)return;const c=data.config||{},published=c.publishVenue??c.venueAnnounced;if(!published||!c.venueName||(!c.mapsUrl&&(!c.latitude||!c.longitude))){tab("venue");return toast("Save and publish venue first.",true)}preview(r);if(r.announcementStatus!=="Sent"){const ok=await mark(r,"Sent");if(!ok)return}const target=`https://wa.me/${phone(r.mobileNumber)}?text=${encodeURIComponent(msg(r))}`;const opened=window.open(target,"_blank","noopener");if(!opened)location.href=target}
async function mark(r,status,card){try{const j=await post({mode:"markAnnouncement",adminKey:key,rsvpId:r.rsvpId,status});if(j.success){r.announcementStatus=j.status;if(status==="Sent"&&card){card.classList.add("fade-out");setTimeout(()=>{messages();guests();eventStatus();filterGuests()},180)}else{messages();guests();eventStatus();filterGuests()}return true}toast(j.message||"Unable to update venue message status.",true)}catch(e){toast(e.message||"Unable to update venue message status.",true)}return false}
...
function openDeleteRsvp(r){if(!r)return;deleting={mode:"deleteRsvp",rsvpId:r.rsvpId};const personalized=!!r.inviteToken;$("deleteDialogTitle").textContent=personalized?"Delete RSVP and Invitation?":"Delete RSVP?";$("deleteDialogBody").textContent=personalized?"This will permanently remove the RSVP, personalized invitation and invitation link from the records. This action cannot be undone.":"This will permanently remove this RSVP and its guest details from the records. This action cannot be undone.";$("confirmDeleteBtn").textContent=personalized?"Delete Permanently":"Delete RSVP";$("deleteError").textContent="";$("deleteDialog").showModal()}
function openDeleteInvitation(inv){if(!inv)return;const pending=inv.status==="Awaiting RSVP"&&!inv.rsvpId;deleting={mode:"deleteInvitation",inviteToken:inv.inviteToken||inv.token};$("deleteDialogTitle").textContent=pending?"Delete Invitation?":"Delete RSVP and Invitation?";$("deleteDialogBody").textContent=pending?"This will permanently remove this personalized invitation and invalidate its invitation link.":"This will permanently remove the RSVP, personalized invitation and invitation link from the records. This action cannot be undone.";$("confirmDeleteBtn").textContent=pending?"Delete Invitation":"Delete Permanently";$("deleteError").textContent="";$("deleteDialog").showModal()}
async function confirmDelete(){if(!deleting)return;const button=$("confirmDeleteBtn");button.disabled=true;button.textContent="Deleting...";$("deleteError").textContent="";try{const payload=deleting.mode==="deleteRsvp"?{mode:"deleteRsvp",adminKey:key,rsvpId:deleting.rsvpId}:{mode:"deleteInvitation",adminKey:key,inviteToken:deleting.inviteToken};const r=await post(payload);if(!r.success)throw Error(r.message||"Unable to delete record.");$("deleteDialog").close();deleting=null;await load();tab("guests");toast(r.message||"Record deleted.")}catch(e){$("deleteError").textContent=e.message||"Unable to delete record."}finally{button.disabled=false;if(deleting)button.textContent=deleting.mode==="deleteInvitation"?"Delete Invitation":"Delete RSVP"}}
