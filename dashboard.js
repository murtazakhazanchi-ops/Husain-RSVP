...
function invitations(){
  ...
  filteredInvitations=showInvitations?items.filter(inv=>{const s=`${inv.guestName} ${inv.mobileNumber} ${inv.inviteToken||inv.token} ${inv.rsvpId}`.toLowerCase(),share=normalizeInviteShare(inv);return(!q||s.includes(q))&&(f==="all"||inv.status===f)&&(sf==="all"||share===sf)}):[];
...
async function nativeShareInvitation(inv){if(!inv)return;const u=requireInvitationUrl(inv);if(!u)return;try{if(!navigator.share)throw Error("native-share-unavailable");const response=await fetch(INVITATION_IMAGE_PATH,{cache:"no-store"});if(!response.ok)throw Error("image-unavailable");const blob=await response.blob(),file=new File([blob],"husain-invite.jpg",{type:blob.type||"image/jpeg"});if(!navigator.canShare||!navigator.canShare({files:[file]}))throw Error("file-share-unavailable");await navigator.share({files:[file],text:approvedInvitationMessage(u)});await markInvitationShared(inv)}catch(e){if(e&&e.name==="AbortError")return;await shareInvitationWhatsApp(inv)}}
